...
@Suppress("unused", "EnumEntryName", "ClassName", "DuplicatedCode")
class Sokoban : PApplet() {
    ...
    private fun showLevel(level: Level, showOn: PGraphics?=null) {
        ...
        for (i in 0..<level.widthInTiles) {
            for (j in 0..<level.heightInTiles) {
                ...
                val currentTile: PImage? = if (indexToColorDefinitions[crateTileIndex]!=levelTileCodes[i][j]) {
    ...
    @Suppress("RemoveExplicitTypeArguments", "LocalVariableName")
    override fun setup() {
